...
export default function Index() {
    ...
    const fetchLiveHealthPackages = async () => {
        try{
            const result = await axios.get(apiLinks.getLiveHealthPackages);
            result.data.data.profileTestList.map(profile => {
                setServerPackages(prevData => [...prevData, profile]);
            });
        }
        catch(err){
            console.log(err);
    ...
    return (
        <>  
            {packageUpdateWindow ? 
                <PackageUpdateWindow serverPackages={serverPackages} setPackageUpdateWindow={setPackageUpdateWindow} packageToUpdate={packageToUpdate}/> : 
                <React.Fragment/>
            }

            {testUpdateWindow ? 
                <TestUpdateWindow setTestUpdateWindow={setTestUpdateWindow} testToUpdate={testToUpdate}/> : 
                <React.Fragment/>
            }
            
            {blogUpdateWindow ? 
                <BlogUpdateWindow setBlogToUpdate={setBlogToUpdate} setBlogUpdateWindow={setBlogUpdateWindow} blogToUpdate={blogToUpdate}/> : 
                <React.Fragment/>
            }
            
            {blogInsertWindow ? 
                <BlogInsertWindow setBlogInsertWindow={setBlogInsertWindow}/> : 
                <React.Fragment/>
            }
            
            {addCouponWindow ? 
                <AddCouponWindow setAddCouponWindow={setAddCouponWindow}/> : 
                <React.Fragment/>
            }
                ...
                <Container fluid className="mr-3">
                    ...
                    <div style={{display:(currentContainer === "adminContent1") ? "block" : "none"}} id="adminContent1" 
                        className="admin-content admin-content1">
                            Dashboard
                            {/* <img src="https://aptdiagnostics-test.s3.ap-south-1.amazonaws.com/f9a9302278622a2fa1c88285214fb3a1"></img> */}
                    </div>


                    <div style={{display:(currentContainer === "adminContent2") ? "block" : "none"}} id="adminContent2" 
                        className="admin-content admin-content2">
                        <Row>
                            <Col>      
                                <FormGroup>
                            ...
                            <FormGroup>
                                ...
                                <IconButton id = "addPackageBtn" onClick={()=>{setPackageUpdateWindow(true)}}>
                                    <AddIcon style={{color:"#ff6363",width:"30px",height:"30px"}}/>
                                </IconButton>
                            </FormGroup>
                            </Col>
                        </Row>
                    ...
                    <div style={{display:(currentContainer === "adminContent4") ? "block" : "none"}} id="adminContent4" 
                        className="admin-content admin-content4">
                        Hello happy birthday
                    </div>


                    <div style={{display:(currentContainer === "adminContent5") ? "block" : "none"}} id="adminContent5" 
                        className="admin-content admin-content5">
                        <Row>
                            <Col>
                    ...
                    <div style={{display:(currentContainer === "adminContent6") ? "block":"none"}} id="adminContent6" 
                        className="admin-content admin-content6">
                        <Card>
                            <CardBody>
                            <Row>
                                <Col md="6">
                    ...
                    <div style={{display:(currentContainer === "adminContent7") ? "block" : "none"}} id="adminContent7" 
                        className="admin-content admin-content7">
                        <Card>
                            <CardBody>
